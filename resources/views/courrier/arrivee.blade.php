            ...
            <form method="POST" action="{{ route('courrier.arrive.phase1.store', ['espace' => $espace]) }}"
                ...
                <div class="card-body">
                            ...
                            <select name="provenance_id" id="provenance_id" class="form-select select2" required>
                                ...
                                @foreach ($provenances as $prov)
                                    <option value="{{ $prov->id }}" data-type="{{ strtolower($prov->type) }}"
                                        {{ old('provenance_id') == $prov->id ? 'selected' : '' }}>
                                        {{ ucfirst($prov->type) }}
                                    </option>
                                @endforeach
                            </select>
                    ...
                    <div class="row mb-3 d-none" id="agent-fields">
                        ...
                        <div class="col-lg-3">
                            <input type="text" name="agent_prenom" class="form-control"
                                value="{{ old('agent_prenom') }}">
                        </div>
                    </div>
                    ...
                    <div class="row mb-3 d-none" id="agent-fields2">
                        ...
                        <div class="col-lg-3">
                            <input type="text" name="agent_matricule" class="form-control"
                                value="{{ old('agent_matricule') }}">
                        </div>
                    </div>
                    ...
                    <div class="row mb-3 d-none" id="etablissement-fields">
                        ...
                        <div class="col-lg-6">
                            <input type="text" name="etablissement_raison_sociale" class="form-control"
                                value="{{ old('etablissement_raison_sociale') }}">
                        </div>
                    </div>
                            ...
                            <select name="objet_id" class="form-select select2" id="objet_id">
                                ...
                                @foreach ($objets as $objet)
                                    <option value="{{ $objet->id }}"
                                        {{ old('objet_id') == $objet->id ? 'selected' : '' }}>
                                        {{ $objet->nom }}
                                    </option>
                                @endforeach
                            </select>
                    ...
                    <div class="text-end mt-4">
                        ...
                        <a href="{{ route('courrier.arrive.historique', ['espace' => $espace]) }}"
                            class="btn btn-secondary ms-2">📂 Historique</a>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            if (btnPrint) {
                btnPrint.addEventListener('click', function() {
                    ...
                    const agentPrenom = document.querySelector('input[name="agent_prenom"]')?.value
                    ?.trim() || '';
      ...
      `;
                    ...
                    if (!printWindow || printWindow.closed || typeof printWindow.closed === 'undefined') {
                        alert(
                            "Le navigateur a bloqué l'ouverture de la fenêtre. Veuillez autoriser les popups.");
                        return;
                    }
