        ...
        <div class="card shadow-sm">
            <div class="card-header bg-success text-white d-flex justify-content-between align-items-center">
                ...
                <div>
                    <a href="{{ route('courrier.depart.historique', ['espace' => $espace]) }}"
                        class="btn btn-outline-light btn-sm me-2">Consultation d'historique</a>
            ...
            <div class="card-body">
                ...
                <form
                    ...
                    <div class="row mb-3">
                        <div class="col-lg-6">
                            ...
                            <input type="date" id="date_envoi" name="date_envoi" class="form-control" required
                                value="{{ old('date_envoi', isset($courrier) && $courrier->date_envoi ? $courrier->date_envoi->format('Y-m-d') : '') }}">
                            @error('date_envoi')
                                <div class="text-danger small mt-1">{{ $message }}</div>
                            @enderror
                        </div>
                    ...
                    <div class="row mb-3">
                        <div class="col-lg-6">
                            <label for="source_type" class="form-label fw-bold">Source <span
                                    class="text-danger">*</span></label>
                            <select id="source_type" name="source_type" class="form-select select2" required>
                                <option value="">-- Sélectionnez la source --</option>
                                <option value="agent"
                                    {{ old('source_type', isset($courrier) && $courrier->nom_agent ? 'agent' : '') == 'agent' ? 'selected' : '' }}>
                                    Agent</option>
                                <option value="departement"
                                    {{ old('source_type', isset($courrier) && $courrier->departement_source_id ? 'departement' : '') == 'departement' ? 'selected' : '' }}>
                                    Département</option>
                            </select>
                            @error('source_type')
                                <div class="text-danger small mt-1">{{ $message }}</div>
                        ...
                        <div class="col-lg-6" id="source_agent" style="display: none;">
                            ...
                            <input type="text" id="agent_nom" name="agent_nom" class="form-control"
                                value="{{ old('agent_nom', $courrier->nom_agent ?? '') }}">
                            @error('agent_nom')
                                <div class="text-danger small mt-1">{{ $message }}</div>
                            @enderror
                        </div>

                        <div class="col-lg-6" id="source_departement" style="display: none;">
